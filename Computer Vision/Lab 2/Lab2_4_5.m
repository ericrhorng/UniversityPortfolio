...
k=4;
NewMean=zeros(k,3);
DistanceMatrix=ones(size(Image,1)*size(Image,2),k)*(255^2);

rng('shuffle');
RandomIdx=randi(size(Image,1),1,1);%Get Random Row
RandomIdy=randi(size(Image,2),1,1); %Get Random Column
CurrentColor=double(impixel(Image,RandomIdy,RandomIdx)); %Get First Random Means
ShapedImage=double(reshape(Image,[size(Image,1)*size(Image,2) 3]));

for i=1:k
    %Calculate color distance for each pixel
    for x=1:size(Image,1)*size(Image,2)
        DistanceMatrix(x,i)=sqrt((CurrentColor(1)-ShapedImage(x,1))^2+(CurrentColor(2)-ShapedImage(x,2))^2+(CurrentColor(3)-ShapedImage(x,3))^2);
    end
    DistanceMatrix(:,i)=min(DistanceMatrix')'; %Calculate min value
    Probability=cumsum((DistanceMatrix(:,i).^2)/sum(DistanceMatrix(:,i).^2)); %Compute Probability
    CurrentColor=ShapedImage(find(Probability>=rand,1),:);
    NewMean(i,:)=CurrentColor;
end
